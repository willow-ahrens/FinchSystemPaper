...
\begin{figure}
    ...
    \vspace{-8pt}
\end{figure}
\kyle{In this paragraph, I wonder if it should be more "our world is full of structured data" which can be described by structured arrays. The "X describes Y" framing feels less fundamental than "Y is Z" like "images, masks, geometry, and transactions consist of long runs of identical values that can be compressed via run length encoding". This might be overly clunky though.}
...
This advances the state-of-the-art over the Looplets work. While Looplets presented a way to merge iterators over multiple single dimensional structures, we go further, combining single-dimensional looplet structures into multi-dimensional tensors and operate on them in a programming language with fully-featured control flow.
