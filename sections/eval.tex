...
\begin{align*}
    ...
    \finchrelabel(expr, idxs...) \quad\quad\quad \finchmapjoin(op, exprs...) \quad\quad\quad \finchaggregate(op, expr)\\
    \finchtable(tns, idxs...)  \quad\quad\quad\quad\quad \finchalias(name) \quad\quad\\
     expr:= \finchreorder | \finchrelabel |\finchmapjoin | \finchaggregate | \finchtable | \finchalias\quad\quad
\end{align*}

Given this language, we now describe how to define a few example functions from the API as Finch Logic expressions, 
\begin{align*}
