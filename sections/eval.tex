...
To get a sense of the Python Array API, we provide a few examples of required functions which we will then translate to Finch Logic statements. 


\subsubsection{Finch Logic}

\begin{align*}
    \finchplan(queries...) &  \finchquery(name, expr) & \finchreorder(expr, idxs...)\\
    \finchrelabel(expr, idxs...)& \finchmapjoin(op, exprs...) & \finchaggregate(op, expr)\\
    & \finchtable(tns, idxs...) & \\
\end{align*}
