...
inclusions to the language at the level of statements includes \mintinline{julia}{for},
...
%
Though the user may doubt the coverage of this approach, the next section will detail how the compiler adds additional features on top of this core language so that more complex control flow (complex index access expressions involving indirection and arithemtic)
...
% \end{multline*}
    ...
    \rightarrow val'}
    \hypo{var\not\in e}
    \infer2[$Define$]{\splitfrac{\langle\finchdefine(var, val, body), (e, t)\rangle}{\rightarrow \langle body, (e[var \mapsto val'], t) \rangle}}
    \end{prooftree}
    \hfill
    \begin{prooftree}
    ...
    \begin{prooftree}
    \hypo{\langle head,s_{1} \rangle \rightarrow s_2}
    ...
    \infer5[$Declare$]{\langle \finchdeclare(tns, init, dims), (e, t)\rangle \rightarrow (e [\finchmode(tns) \mapsto update], \llangle declare(tns', init', dims'...), t \rrangle)}
    \end{prooftree}
    \vspace{6pt}
