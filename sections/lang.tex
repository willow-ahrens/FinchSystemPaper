...
% \end{multline*}
    ...
    \infer5[$Declare$]{\langle \finchdeclare(tns, init, dims), (e, t)\rangle \rightarrow (e [\finchmode(tns) \mapsto \finchupdate], \llangle declare(tns', init', dims'...), t \rrangle, d\cup\{ tns\})}
    \end{prooftree}
    \vspace{6pt}
    ...
    \hypo{tns\in d}
    \hypo{e(tns) = tns'}
    \infer4[$Freeze$]{\langle\finchfreeze(tns), (e, t)\rangle \rightarrow (e [\finchmode(tns) \mapsto \finchread], \llangle freeze(tns'), t \rrangle, d)}
    \end{prooftree}
    \vspace{6pt}
...
\begin{prooftree}
    \infer0[$Phase$]{\splitfrac{\langle \finchloop(i, \finchextent(a, b), E[\finchaccess(phase(extent(c, d), body), j..., i)]), s\rangle}{\rightarrow \langle \text{\finchloop}(i, extent(max(a, c), min(b, d)), E[\text{\finchaccess}(body, j..., i)]), s\rangle}}
\end{prooftree}
\vspace{6pt}
...
\begin{prooftree}
    ...
    \infer1[$StepperSeek$]{\splitfrac{\langle \finchloop(i, extent(a, b), E[\finchaccess(stepper(seek, body, next), j..., i)]), (e, t, d)\rangle}{\rightarrow \langle \finchloop(i, extent(a, b), E[\finchaccess(stepper(body, next), j..., i)]), (e, t', d)\rangle}}
\end{prooftree}
\vspace{6pt}
...
%    \vspace{6pt}
    ...
    \begin{prooftree}
    \infer0[$Loop$]{\splitfrac{\langle\finchloop(i, \finchextent(a, b), body), s\rangle}{\rightarrow \finchblock(\finchdeclare(i, a, body), \finchsieve(a < b, \finchloop(i, \finchextent(a + 1, b), body)), s\rangle}}
    \end{prooftree}
