...
Our world is full of structured arrays.  Sparse arrays (which store only nonzero
elements) can describe networks and simulations. Run-length encoding describes
images and masks, geometry, and databases (such as a list of transactions with
the date field all the same). Symmetry, bands, padding, and blocks are all
natural matrix properties which arise in the course of scientific computing or
simulations of the natural world. Operators, such as convolution, can be
expressed as structured arrays. For example, a convolution with a filter can be
expressed as a matrix multiplication with the toeplitz matrix of all the
circular shifts of the filter.
...
%Existing solutions do not scale to the complexity of the problem
% Must choose between program flexibility and datastructure flexibility
% Dense arrays have tons of programming constructs
% Sparse array frameworks have very limited functionality, discuss a few key related works

% Finch Supports both! The combination of the two leads us to a new data-structure driven programming model.
% In this model, we can search the space of programs more efficiently, and we can more efficiently search the space of datastructures.
% Programs are specified separately from datastructures.
% Certain concepts are expressed in programs, others in data
% At a high level, whenever we need to match corresponding keys in a program, those keys become array indices, and the datastructure becomes a sparse array
...
%
%- We’re bringing these together, because if both of these can work together, we can more efficiently search the space of programs
